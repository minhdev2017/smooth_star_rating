...
class _SmoothStarRatingState extends State<SmoothStarRating> {
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: Wrap(alignment: WrapAlignment.start, spacing: widget.spacing, children: List.generate(widget.starCount, (index) => buildStar(context, index))),
    );
  }
  ...
  Widget buildStar(BuildContext context, int index) {
    ...
    } else if (index > widget.rating - (widget.allowHalfRating ? halfStarThreshold : 1.0) && index < widget.rating) {
      icon = Icon(
        ...
        : kIsWeb
                ...
                child: GestureDetector(
                  onTapDown: (detail) {
                    ...
                    if (widget.onRated != null) {
                      widget.onRated(normalizeRating(widget.rating));
                    }
                  },
                  onHorizontalDragUpdate: (dragDetails) {
                    ...
                    debounceTimer = Timer(Duration(milliseconds: 100), () {
                      if (widget.onRated != null) {
                        widget.rating = normalizeRating(newRating);
                        widget.onRated(widget.rating);
                      }
                    }); */
                  },
                  child: icon,
            ...
            : GestureDetector(
                ...
                onHorizontalDragEnd: (e) {
                  if (widget.onRated != null) widget.onRated(widget.rating);
                },
                onHorizontalDragUpdate: (dragDetails) {
                  RenderBox box = context.findRenderObject();
                  var _pos = box.globalToLocal(dragDetails.globalPosition);
